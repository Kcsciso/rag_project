...
**背景**: v31 引入 MinerU 解析 JAKA 手册产出 Markdown 后[cite: 3]，文档内存在 245 张图片、17 处 HTML 表格以及 306 处单井号标题。直接切片会导致切片碎裂、参数丢失与 HTML 污染。本阶段完成了**双模型微服务架构搭建与多模态切片提纯管线**。

---

#### 核心工作与连环排雷记录

1. **Qwen2-VL-7B 视觉语言模型服务化 (:8005)**:
   - 依赖对齐：锁定 `torch==2.4.0`、`vllm==0.6.2`、`transformers==4.45.2`、`cudnn==9.1.0.70`，排除 CUDA 13 依赖污染。
   - 显存自适应：在 A100 GPU 1 存在 12.5GB 常驻占用的环境下，配置 `--gpu-memory-utilization 0.5`、`--max-num-seqs 16`、`max_pixels: 602112` 并启用 `--enforce-eager`，实现 27.5GB 剩余显存下稳定运行。
   - 修复 RoPE 断言崩溃：配置 `config.json` 显式声明 `"mrope_section": [16, 24, 24]`。

2. **HTML 表格规整转换管道 (`clean_html_tables`)**:
   - 彻底将 17 处 HTML `<table>` 转换为标准 Markdown 表格，并自动补齐缺失单元格，防止 HTML 标签破坏 Dense 向量语义[cite: 3]。

3. **三重图片防线与防废话 VLM 提纯 (`markdown_loader.py`)**:
   - **几何防线**：过滤 `<80px` 及长宽比 `>8` 的小图标与装饰线条。
   - **语义防线**：扫描图片前后 100 字符，校验是否包含“图/表/参数/设置/配置/网络/界面”等图注关键词。
   - **防模板化 Prompt 优化**：针对模型初期输出大量“坐标: 未提供”等模板废话的问题，压低采样温度至 `0.05` 并强制约束“仅提取事实，纯图回复‘仅为UI示意图’，严禁占位项”，成功提取出 Modbus 端口、低通滤波器频率 (0.000 Hz)、负载设置等干货数据。

4. **切片可视化与自动化质检工具 (`inspect_chunks.py`)**:
   - 提供章节层级总览表、IP/端口实体检索及切片卡片展开功能，全量 244 个切片实现 0 HTML 残留与高信噪比结构化存储（`data/jaka_manual_chunks.json`）。
...
| 维度 | v31 (旧) | v32 (新) | 变化 |
|------|---------|---------|------|
| 图片处理 | 离线静态图片链接 | Qwen2-VL-7B 自动识别提纯为 Markdown 注入正文 | 视觉参数 100% 文本化 |
| 表格处理 | 原始 HTML `<table>` 标签残留 | 正则清洗为标准 Markdown 对齐表格 | 消除 HTML 语义噪声 |
| 标题切片 | 遇 `#` 粗暴切断（导致切片碎裂） | 智能章节编号识别 + 面包屑层级栈维护 | 语义连贯性保全 |
| 数据持久化 | 仅中间 Markdown 文件 | 结构化 JSON 落盘 (`jaka_manual_chunks.json`) | 解耦切片与 Embedding 阶段 |
| 质检手段 | 终端手动查看 | 交互式质检工具 (`inspect_chunks.py`) | 可视化质检与快速验证 |
